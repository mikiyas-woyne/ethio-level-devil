...
export const GameCanvas: React.FC<GameCanvasProps> = ({
  ...
  const updatePhysics = () => {
    ...
    cratesRef.current.forEach((crate) => {
      ...
      // Check overlap with players (Push mechanics)
      playersRef.current.forEach((p) => {
        if (!p.isAlive) return;

        if (isOverlapping(p.x, p.y, p.width, p.height, crate.x, crate.y, crate.width, crate.height)) {
          // Push logic based on intersection horizontal side
          const pMid = p.x + p.width / 2;
          const cMid = crate.x + crate.width / 2;

          if (pMid < cMid) {
            // Push right
            const targetX = crate.x + 2;
            if (!checkSolidOverlap(targetX, crate.y, crate.width, crate.height)) {
              crate.x = targetX;
            }
          } else {
            // Push left
    ...
    playersRef.current.forEach((p, idx) => {
      ...
      if (!p.isP2) {
        ...
        // Player 2: Arrow keys
        keys.left = keysPressedRef.current['arrowleft'] || keysPressedRef.current['j'];
        keys.right = keysPressedRef.current['arrowright'] || keysPressedRef.current['l'];
        keys.jump = keysPressedRef.current['arrowup'] || keysPressedRef.current['i'];
      }
      ...
      const gravityStrength = isGravityFlippedRef.current ? -0.22 : 0.22;
      p.vy += gravityStrength;

      // Clamp max vertical fall speed
      const terminalSpeed = 5;
      ...
      if (keys.jump && p.isGrounded) {
        audio.playSFX('jump');
        p.vy = isGravityFlippedRef.current ? 4.2 : -4.2;
        p.isGrounded = false;
      }
      ...
      const centerTileX = Math.floor((p.x + p.width / 2) / 16);
      const centerTileY = Math.floor((p.y + p.height / 2) / 16);

      if (centerTileX >= 0 && centerTileX < 30 && centerTileY >= 0 && centerTileY < 17) {
        const tileType = currentGridRef.current[centerTileY][centerTileX];

        // Spike collisions (type 3: UP, 4: DOWN, 5: LEFT, 6: RIGHT)
        ...
        if (level.invisiblePits) {
          level.invisiblePits.forEach((pit) => {
            if (pit.x === centerTileX && pit.y === centerTileY) {
              // Stepped on an invisible pit! Drop block immediately
              if (currentGridRef.current[centerTileY][centerTileX] === 1) {
                currentGridRef.current[centerTileY][centerTileX] = 0; // instantly empty!
                audio.playSFX('death');
                createSparkles(centerTileX * 16 + 8, centerTileY * 16 + 8, '#D84315');
              }
            }
          });
        }

        // Crumbling tiles (Floor 1 shakes and disintegrates on stand)
        if (tileType === 1 || tileType === 17) {
          const isSteppedOn = isGravityFlippedRef.current
            ? p.y <= (centerTileY + 1) * 16 && p.y >= centerTileY * 16
            : p.y + p.height >= centerTileY * 16 && p.y + p.height <= (centerTileY + 1) * 16;

          if (isSteppedOn) {
            // Check if already crumbling
            const activeCrumble = crumblingTilesRef.current.find(
              (ct) => ct.tileX === centerTileX && ct.tileY === centerTileY
            );
            if (!activeCrumble) {
        ...
        if (tileType === 10 && isKeyAvailableRef.current) {
          ...
          audio.playSFX('ding');
          createSparkles(centerTileX * 16 + 8, centerTileY * 16 + 8, '#FFD700');
        ...
        if (tileType === 11) {
          currentGridRef.current[centerTileY][centerTileX] = 0; // collect
          audio.playSFX('ding');
          createSparkles(centerTileX * 16 + 8, centerTileY * 16 + 8, '#FFD700');
        }
        ...
        if (tileType === 12) {
          currentGridRef.current[centerTileY][centerTileX] = 0; // collect fake
          createSparkles(centerTileX * 16 + 8, centerTileY * 16 + 8, '#B0BEC5');
          // No sound, no key progress, purely disappears
        }
        ...
        if (tileType === 8) {
          ...
          p.isGrounded = false;
          audio.playSFX('jump');
          createSparkles(centerTileX * 16 + 8, centerTileY * 16 + 12, '#4CAF50');
        }
  ...
  const renderGame = () => {
    ...
    trapsRef.current.forEach((trap) => {
      ...
      if (trap.type === 'buzzsaw') {
        ...
        ctx.rotate((activeFrameRef.current * 8 * Math.PI) / 180);

        const r = trap.radius || 20;
        ...
        for (let i = 0; i < numSpikes; i++) {
          ...
          ctx.fillRect(-spikeW * 0.5, r * 0.8, spikeW, r * 0.2);
          ctx.fillStyle = "#D6D6D6";
          ctx.beginPath();
          ctx.moveTo(-spikeW * 0.5, r * 0.95);
          ctx.lineTo(0, r + spikeLen);
          ctx.lineTo(spikeW * 0.5, r * 0.95);
          ctx.closePath();
          ctx.fill();
          ctx.restore();
        }
        ...
        ctx.fillStyle = '#9E642B';
        ctx.fill();

        // 4. Gold Star Emblem
        const starOuter = r * 0.5;
        const starInner = r * 0.25;
        ctx.beginPath();
        for (let i = 0; i < 16; i++) {
